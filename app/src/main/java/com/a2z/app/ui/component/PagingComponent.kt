...
@Composable
fun <T> AppLazyList(pagingData : LazyPagingItems<*>, content :  @Composable (T)->Unit) {
    ...
    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.BottomCenter){
        ...
        backgroundColor = RedColor) {
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(8.dp)) {
                Icon(imageVector = Icons.Default.Warning, contentDescription = "Error", tint = Color.White)
                Spacer(modifier = Modifier.width(8.dp))
                Text(text = "Error occurred!", color = Color.White)
                Spacer(modifier = Modifier.weight(1f))
                TextButton(onClick = { pagingData.retry() }, colors =  ButtonDefaults.textButtonColors(
