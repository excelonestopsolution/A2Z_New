package com.a2z_di.app.util.ui
...
class DateEditText : AppCompatEditText, TextWatcher {
    ...
    override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
        ...
        preLength = curLength
        curLength = s.toString().length

        val mText = s.toString()
        val mTextLength = mText.length


        if (mTextLength == 3) {
            if(curLength > preLength) {
                val textWithForwardSlash = textWithForwardSlash(mText)
                setText(textWithForwardSlash)
                setSelection(textWithForwardSlash.length)
            }
        }
        if (mTextLength == 6) {
            if(curLength>preLength) {
    ...
    private fun textWithForwardSlash(text: String): String {
        val lastLetter = text.substring(text.length - 1)
        val newText = text.substring(0, text.length - 1)
        return "$newText/$lastLetter"
    }
